...
class CNN_Transformer(nn.Module):
    def __init__(self, num_hiddens, ffn_num_hiddens, num_heads, num_blks,
                dropout, use_bias=False, **kwargs):
        super(CNN_Transformer, self).__init__(**kwargs)
        self.conv1 = nn.Conv1d(1, 32, kernel_size=19, padding=5, stride=5)
        self.bn1 = nn.BatchNorm1d(32)
        self.relu = nn.ReLU()
        self.pool = nn.MaxPool1d(2, padding=1, stride=2)
        self.conv2 = nn.Conv1d(32, num_hiddens, kernel_size=3, padding=1)

        self.transformer_encoder = TransformerEncoder(num_hiddens, [300, num_hiddens], 
                ffn_num_hiddens, num_heads, num_blks, dropout, use_bias)
        self.linear1 = nn.Linear(num_hiddens * 300, 2)
        self.relu = nn.ReLU()
...
if __name__ == '__main__':
    ...
    model = CNN_Transformer(512, 2048, 8, 6, 0.1, True)
    output = model(input)